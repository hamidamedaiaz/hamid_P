...
public class RestaurantDishManagementStepDefinitions {
    ...
    public void theRestaurantMenuIsEmpty() {
    ...
    public void theMenuContainsAvailableDish(String name) {
    ...
    public void theMenuContainsDish(String name) {
        theMenuAlreadyContainsADish(name);
    }
    ...
    public void adminModifiesName(String newName) {
        updateAndStore(new UpdateDishRequest(restaurantId, lastDishId, newName, null, null, null, null));
    }

    @When("the administrator modifies the dish description to {string}")
    public void adminModifiesDescription(String newDesc) {
        updateAndStore(new UpdateDishRequest(restaurantId, lastDishId, null, newDesc, null, null, null));
    }

    @When("the administrator modifies the dish price to {string}")
    public void adminModifiesPrice(String newPrice) {
        updateAndStore(new UpdateDishRequest(restaurantId, lastDishId, null, null, new BigDecimal(newPrice), null, null));
    }

    @When("the administrator modifies the dish category to {string}")
    public void adminModifiesCategory(String newCat) {
        updateAndStore(new UpdateDishRequest(restaurantId, lastDishId, null, null, null, DishCategory.valueOf(newCat), null));
    }

    @When("the administrator makes the dish {string} unavailable")
    public void adminMakesUnavailable(String ignored) {
        updateAndStore(new UpdateDishRequest(restaurantId, lastDishId, null, null, null, null, false));
    }

    @When("the administrator makes the dish {string} available")
    public void adminMakesAvailable(String ignored) {
        updateAndStore(new UpdateDishRequest(restaurantId, lastDishId, null, null, null, null, true));
    }
    ...
    public void dishPresent(String name) {
        refreshRestaurant();
        assertTrue(restaurant.findDishByName(name).isPresent());
    }

    @Then("the dish should be available for ordering")
    public void dishAvailable() {
        refreshRestaurant();
        assertTrue(restaurant.findDishById(lastDishId).get().isAvailable());
    }

    @Then("the menu should contain {int} dish(es)")
    public void menuShouldContain(int count) {
        refreshRestaurant();
        assertEquals(count, restaurant.getMenu().size());
    }

    @Then("the menu should contain {int} dish")
    public void menuDish(int c) {
        menuShouldContain(c);
    }

    @Then("the menu should still contain {int} dish")
    public void menuStill(int c) {
        menuShouldContain(c);
    }

    @Then("the dish addition should be rejected")
    public void additionRejected() {
        assertTrue(operationFailed);
        assertFalse(addResponse.success());
    }

    @Then("the error message should contain {string}")
    public void errorContains(String expected) {
        assertTrue(errorMessage.toLowerCase().contains(expected.toLowerCase()));
    }

    @Then("the dish should have name {string}")
    public void dishHasName(String n) {
        refreshRestaurant();
        assertEquals(n, restaurant.findDishById(lastDishId).get().getName());
    }

    @Then("the dish should have description {string}")
    public void dishHasDesc(String d) {
        refreshRestaurant();
        assertEquals(d, restaurant.findDishById(lastDishId).get().getDescription());
    }

    @Then("the dish should have price {string}")
    public void dishHasPrice(String p) {
        refreshRestaurant();
        assertEquals(new BigDecimal(p), restaurant.findDishById(lastDishId).get().getPrice());
    }

    @Then("the dish should have category {string}")
    public void dishHasCat(String c) {
        refreshRestaurant();
        assertEquals(DishCategory.valueOf(c), restaurant.findDishById(lastDishId).get().getCategory());
    }

    @Then("the modification should be confirmed")
    public void modificationConfirmed() {
        assertFalse(operationFailed);
        assertTrue(updateResponse.success());
    }

    @Then("the dish {string} should be marked as unavailable")
    public void dishUnavailable(String n) {
        refreshRestaurant();
        assertFalse(restaurant.findDishById(lastDishId).get().isAvailable());
    }

    @Then("the dish should remain in the menu")
    public void dishRemain() {
        refreshRestaurant();
        assertTrue(restaurant.findDishById(lastDishId).isPresent());
    }

    @Then("the dish {string} should be marked as available")
    public void dishAvailableAgain(String n) {
        refreshRestaurant();
        assertTrue(restaurant.findDishById(lastDishId).get().isAvailable());
    }

    @Then("the dish {string} should no longer be present in the menu")
    public void dishRemoved(String n) {
        refreshRestaurant();
        assertTrue(restaurant.findDishByName(n).isEmpty());
    }

    @Then("the menu should be empty")
    public void menuEmpty() {
        assertTrue(restaurant.getMenu().isEmpty());
    }

    @Then("the removal should be confirmed")
    public void removalConfirmed() {
        assertFalse(operationFailed);
        assertTrue(removeResponse.success());
    }

    @Then("the removal should be rejected")
    public void removalRejected() {
        assertTrue(operationFailed);
        assertFalse(removeResponse.success());
    }

    @Then("the modification should be rejected")
    public void modificationRejected() {
        assertTrue(operationFailed);
        assertFalse(updateResponse.success());
    }
}
